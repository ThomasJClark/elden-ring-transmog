...
namespace SoulsFormats
{
    public partial class HKX
    {
        ...
        public class CompressedMeshBVHNode : IHKXSerializable
        {
            ...
            {
                float x =
                    ((float)(BBX >> 4) * (float)(BBX >> 4))
                        * (1.0f / 226.0f)
                        * (parentMax.X - parentMin.X)
                    + parentMin.X;
                float y =
                    ((float)(BBY >> 4) * (float)(BBY >> 4))
                        * (1.0f / 226.0f)
                        * (parentMax.Y - parentMin.Y)
                    + parentMin.Y;
                float z =
                    ((float)(BBZ >> 4) * (float)(BBZ >> 4))
                        * (1.0f / 226.0f)
                        * (parentMax.Z - parentMin.Z)
                    + parentMin.Z;
            ...
            {
                float x =
                    -((float)(BBX & 0x0F) * (float)(BBX & 0x0F))
                        * (1.0f / 226.0f)
                        * (parentMax.X - parentMin.X)
                    + parentMax.X;
                float y =
                    -((float)(BBY & 0x0F) * (float)(BBY & 0x0F))
                        * (1.0f / 226.0f)
                        * (parentMax.Y - parentMin.Y)
                    + parentMax.Y;
                float z =
                    -((float)(BBZ & 0x0F) * (float)(BBZ & 0x0F))
                        * (1.0f / 226.0f)
                        * (parentMax.Z - parentMin.Z)
                    + parentMax.Z;
                return new Vector3(x, y, z);
        ...
        public class CollisionMeshChunk : IHKXSerializable
        {
            ...
            private BVHNode buildBVHTree(Vector3 parentBBMin, Vector3 parentBBMax, uint nodeIndex)
            {
                ...
                if ((cnode.IDX & 0x01) > 0)
                {
                    ...
                    node.Right = buildBVHTree(
                        node.Min,
                        node.Max,
                        nodeIndex + ((uint)cnode.IDX & 0xFE)
                    );
                }
                else
                {
        ...
        public class HKNPCompressedMeshShapeData : HKXObject
        {
            ...
            )
            {
                ...
                packedVertices = new HKArray<SmallCompressedVertex>(
                    hkx,
                    section,
                    this,
                    br,
                ...
                sharedVertices = new HKArray<LargeCompressedVertex>(
                    hkx,
                    section,
                    this,
                    br,
            ...
            )
            {
                ...
                sharedVerticesIndex.WriteReferenceData(
                    hkx,
                    section,
                    bw,
                    sectionBaseOffset,
                ...
                primitiveDataRuns.WriteReferenceData(
                    hkx,
                    section,
                    bw,
                    sectionBaseOffset,
            ...
            private BVHNode buildBVHTree(Vector3 parentBBMin, Vector3 parentBBMax, uint nodeIndex)
            {
                ...
                if ((cnode.IDX0 & 0x80) > 0)
                {
                    ...
                    node.Right = buildBVHTree(
                        node.Min,
                        node.Max,
                        nodeIndex + ((((uint)cnode.IDX0 & 0x7F) << 8) | (uint)cnode.IDX1) * 2
                    );
                }
                else
                {
            ...
            public BVHNode getMeshBVH()
            {
                ...
                {
                    root.Left = buildBVHTree(root.Min, root.Max, 1);
                    root.Right = buildBVHTree(
                        root.Min,
                        root.Max,
                        ((((uint)cnode.IDX0 & 0x7F) << 8) | (uint)cnode.IDX1) * 2
                    );
                }
                else
                {
