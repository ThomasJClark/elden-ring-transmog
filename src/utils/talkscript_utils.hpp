...
class talkscript_menu_state : public from::ezstate::state
{
  public:
    ...
    std::array<from::ezstate::transition *, 1> transition_array{&transition};
        ...
        : opts(std::move(opts_arg))
    {
        ...
        entry_events = from::ezstate::events{events.get(), opts.size() + 3};
        transitions = from::ezstate::transitions{transition_array};
        branch_state.transitions =
            from::ezstate::transitions{branch_transitions.get(), branch_transition_count};
    }
};

struct open_regular_shop_state : public from::ezstate::state
{
    std::array<unsigned char, 6> begin_shop_lineup_id;
    std::array<unsigned char, 6> end_shop_lineup_id;
    std::array<from::ezstate::expression, 2> args;
    from::ezstate::event event;
    from::ezstate::transition transition;
    std::array<from::ezstate::transition *, 1> transition_array{&transition};

    open_regular_shop_state(int begin_shop_lineup_id, int end_shop_lineup_id,
                            from::ezstate::state *return_state)
        : from::ezstate::state{.transitions = transition_array, .entry_events = {&event, 1}},
          ...
          event{from::talk_command::open_regular_shop, args},
          transition{return_state, from::ezstate::expression{regular_shop_menu_closed_expression}}
    {
    }
};

struct give_speffect_to_player_state : public from::ezstate::state
{
    std::array<unsigned char, 6> speffect_id;
    std::array<from::ezstate::expression, 1> args_array;
    from::ezstate::event event;
    from::ezstate::transition transition;
    std::array<from::ezstate::transition *, 1> transitions_array{&transition};

    give_speffect_to_player_state(int speffect_id, from::ezstate::state *return_state)
        : from::ezstate::state{.transitions = transitions_array, .entry_events = {&event, 1}},
          speffect_id{make_int_expression(speffect_id)}, args_array{this->speffect_id},
          event{from::talk_command::give_sp_effect_to_player, args_array},
          transition{return_state, from::ezstate::expression{regular_shop_menu_closed_expression}}
    {
    }
};
